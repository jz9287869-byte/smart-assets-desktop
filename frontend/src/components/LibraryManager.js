...
export default function LibraryManager({ onLibraryChange, showToast }) {
  ...
  const handleCreateLibrary = async () => {
    ...
    setIsLoading(true);
    try {
      const result = await window.electronAPI?.libraryAPI?.create?.({ name });
      if (result?.success) {
        closeDialog();
        await loadLibraries();
        onLibraryChange?.(result.library);
        showToast?.('资源库创建成功，现在可以去导入页选择文件夹');
      } else {
        showToast?.(result?.error || '资源库创建失败');
      }
    } catch (error) {
      ...
      {showCreateDialog && (
        <div className="dialog-overlay" onClick={closeDialog}>
          <div className="dialog" onClick={(event) => event.stopPropagation()}>
            <h3 className="mb-1 text-lg font-semibold text-slate-100">新建资源库</h3>
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              >
                {isLoading ? <Loader className="animate-spin" size={16} /> : '创建资源库'}
              </button>
            </div>
          </div>
        </div>
